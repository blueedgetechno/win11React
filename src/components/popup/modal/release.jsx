...
function ReleaseAppModal({ data }) {
	...
	const handleSubmitForm = (e) => {
		...
		const cluster_id = formatEvent(data?.event)?.host?.info?.cluster_id ?? ''
	...
	return (
		<form onSubmit={handleSubmitForm} id="form" className="flex flex-col gap-4 p-[16px]">
			<div className="flex flex-col gap-2 ">
				...
				<label htmlFor="store_id" className="flex gap-8 items-center">
					...
					<select id="store_id" className="flex-1 h-[40px] border border-solid px-2 rounded-md" name="store_id" >
						{
							storeApps.map(app => (
				...
				<label htmlFor="desc" className="flex gap-8 items-center">
					...
					<input className="flex-1 h-[40px] border border-solid px-2 rounded-md" type="text" placeholder="" name="desc" id="desc" />
				</label>

			</div>
			<div className="flex items-center gap-5 mt-4">
				<select id="vol_speed" className="h-[32px] text-[16px]" name="vol_speed" >
					<option value="HOT">HOT</option>
					<option value="WARM">WARM</option>
					<option value="COLD">COLD</option>
				</select>
				<select id="vol_availability" defaultValue={"LA"} className="h-[32px] text-[16px]" name="vol_availability" >
			...
			<div className="flex items-center justify-around gap-6" >
				<select id="gpu_model" defaultValue={"RTX 3060 Ti"} className="h-[32px] text-[16px]" name="gpu_model">
					<option value="RTX 3060 Ti" >RTX 3060 Ti</option>
					<option value="RTX 2060 SUPER">RTX 2060 Super</option>
				</select>
				<label htmlFor="ram">
					Ram:
					<select id="ram" defaultValue={"12"} className="ml-2 h-[32px] text-[16px]" name="ram">
						<optgroup label="RAM">
				...
				<label htmlFor="">
					VCPUS:
					<select id="vcpus" defaultValue={"12"} className="ml-2 h-[32px] text-[16px]" name="vcpus">
			...
			<div className="flex items-center justify-around">
				<label className="flex items-center gap-2" htmlFor="hidevm"> <input type="checkbox" className="h-[20px] w-[20px]" id="hidevm" name="hidevm" /> <span>Hide VM?</span></label>
				<label className="flex items-center gap-2" htmlFor="vdriver"> <input type="checkbox" className="h-[20px] w-[20px]" id="vdriver" name="vdriver" /> <span>Virtual Driver ?</span></label>
			</div>
