...
const Modal = (props) => {
  ...
  return (
    <div>
      ...
      >
        <div style={{background: theme =='dark' ? 'rgb(255 255 255 / 5%)' : ''}} className="flex flex-col bg-[#eff4f9]">
          <button
            className="self-end flex items-center bg-transparent outline-none border-none px-3 py-2 hover:bg-red-500"
            onClick={closeModal}
          >
            <img style={{filter:theme =='dark' ? ' invert(1)' : ''}} className="w-[14px]" src="img/icon/ui/close.png" alt="" />
          </button>
        </div>
