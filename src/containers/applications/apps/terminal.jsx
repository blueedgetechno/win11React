...
export const WnTerminal = () => {
  ...
  const saveHistory = (value) => {
    if (value === null || value === undefined || value == 'clear') return;
  ...
  const cmdTool = async (cmd) => {
    ...
    const runSpecificHistoryCmd = (id) => {
      let record = JSON.parse(localStorage.getItem('history')) || [];
      if (id < 1 || id > record.length) {
        tmpStack.push('No History Found for this id!');
      } else {
        tmpStack.push(`Running command: ${record[id - 1]}`);
        runCMD(record[id - 1]);
      }
    };

    const runCMD = async (type) => {
      if (type == 'echo') {
        if (arg.length) {
          tmpStack.push(arg);
        } else {
        ...
        var helpArr = [
          ...
          'HISTORY-ID     EXECUTES COMMAND FROM HISTORY.',
        ...
        let record = JSON.parse(localStorage.getItem('history')) || [];
        if (record.length === 0)
          tmpStack.push('No commands in history to display.');
        else {
          record.forEach((item, index) => {
            if (item !== null) tmpStack.push(`${index + 1}. ${item}`);
          });
        }
      }
      // history id
  ...
  useEffect(() => {
    let x = stack[lastCmd - 1]?.split('>');
    if (x) saveHistory(x[1]);
  }, [stack, lastCmd]);
