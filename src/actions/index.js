...
import { VirtualOSBrowserCore } from 'virtualos-core/dist/src';
  ...
  if (act == 'open') {
    ...
    if (data.type) {
      ...
      var app = Object.keys(apps).filter(
        (x) => apps[x].action == data.type || (apps[x].payload == data.payload && apps[x].payload != null)
      );
...
export const loginWithGoogle = async () => {
  const user = await core.LoginWithGoogle();
...
export const getUserInfo = async (dispatch) => {
  let user = { name: 'dat', email: 'do' };
  await core.getUserInfor().then((data) => {
    user = data.user;
  });
  console.log(user);
  dispatch({
